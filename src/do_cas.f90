...
subroutine submit_dalton_icss_job(proname, mem, nproc, nfile)
 ...
 character(len=240) :: makefile, molname, dalname
 character(len=240), intent(in) :: proname
 ...
 write(fid,'(/,A,I0)') 'mem = ', mem/2
 write(fid,'(A,I0)') 'np = ', nproc/2
 ...
 i = system('make -j2 -f '//TRIM(makefile)//' all')
 if(i /= 0) then
 ...
 do i = 1, nfile, 1
  ...
  call delete_files(2, [molname, dalname])
 end do ! for i
