...
subroutine molecp2fch(fchname, uhf, nbf_in, nif_in, na_in, nb_in, ncontr_in, &
 nprim_in, charge_in, mult_in, natom_in, LenNCZ_in, ielem_in, ghost, &
 shell_type_in, prim_per_shell_in, shell2atom_map_in, virial_in, tot_e_in, &
 coor_in, prim_exp_in, contr_coeff_in, KFirst_in, KLast_in, Lmax_in, LPSkip_in,&
 NLP_in, RNFroz_in, CLP_in, ZLP_in)
 use fch_content, only : is_uhf, nbf, nif, na, nb, ncontr, nprim, charge, mult,&
  ...
  virial, tot_e, coor, prim_exp, contr_coeff, contr_coeff_sp, KFirst, KLast, Lmax,&
  LPSkip, NLP, RNFroz, CLP, CLP2, ZLP
 integer :: i
...
!f2py depend(nprim_in) :: prim_exp_in, contr_coeff_in
 integer, intent(in) :: KFirst_in(natom_in, 10), KLast_in(natom_in, 10), Lmax_in(natom_in), &
                        LPSkip_in(natom_in)
...
!f2py depend(LenNCZ_in) :: NLP_in, CLP_in, ZLP_in
 ...
 logical, intent(in) :: ghost(natom_in)
