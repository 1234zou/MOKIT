...
subroutine molden2fch(molden, iprog, natorb)
 ...
 call read_nif_from_molden(molden, nif)
 ! Note: the molden file generated by some programs does not print all MO
 ! coefficients of an MO. So the nbf cannot be determined from subroutine
 ! read_nif_from_molden.
...
! read nif from a given .molden file
subroutine read_nif_from_molden(molden, nif)
 implicit none
 integer :: i, fid
 ...
 if(i /= 0) then
  write(6,'(/,A)') 'ERROR in subroutine read_nif_from_molden: no [MO] found in &
                   &file '//TRIM(molden)
  close(fid)
  stop
 end if
 ...
 do while(.true.)
  read(fid,'(A)',iostat=i) buf
  if(i /= 0) exit
  if(INDEX(buf(1:7),TRIM(key)) > 0) nif = nif + 1
 end do ! for while

 close(fid)
 call check_uhf_in_molden(molden, uhf)
 if(uhf) nif = nif/2
end subroutine read_nif_from_molden
