...
program main
 implicit none
 integer :: i, narg, iprog
 integer, parameter :: n = 13
 character(len=3) :: str3
 character(len=7) :: str7
 character(len=7), parameter :: sprog(0:n) = ['--Qbics','-bagel ','-cfour ', &
  '-cp2k  ','-dalton','-et    ','-molcas','-molpro','-mrcc  ','-nwchem', &
  '-orca  ','-psi4  ','-pyscf ','-tm    ']
 ...
 if(narg<2 .or. narg>3) then
  ...
  do i = 3, n, 1
   write(6,'(A,I2,A)') ' Example ',i,': molden2fch a.molden '//TRIM(sprog(i))
  end do ! for j
  write(6,'(1X,A)') "If you are transferring NOs, you can append a '-no' argume&
                    &nt, e.g."
  write(6,'(5X,A,/)') 'molden2fch a.molden -orca -no'
  write(6,'(1X,A)') 'There is also a specially designed option for Qbics-MolSta&
                    &r:'
  write(6,'(5X,A,/)') 'molden2fch a.molden --QbicsMolstar'
  stop
 end if
