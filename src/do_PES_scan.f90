...
subroutine check_scan_val(scan_itype, n, scan_val)
 ...
 do i = 1, n-1, 1
  ...
  case(1) ! scan a bond
   if(rtmp < 0d0) then
    write(iout,'(/,A)') error_warn//' scan values must be in descending order.'
    write(iout,'(A,10F6.3)') 'scan_val=',(scan_val(j),j=1,n)
    stop
   end if
   if(rtmp<1d-3 .or. rtmp>10d0) then
    write(iout,'(/,A)') error_warn//' intervals of the scanned bond are'
    write(iout,'(A)') 'too large or too small. It should be >0.001 and <10.0.'
    stop
   end if
  case(2,3) ! scan an angle or a dihedral
   rtmp = DABS(rtmp)
   if(rtmp<1d0 .or. rtmp>100d0) then
    write(iout,'(/,A)') error_warn//' intervals of the scanned angle/dihedral'
    write(iout,'(A)') 'are too large or too small. It should be >1 and <100.0&
                     & degree.'
    stop
   end if
  case default
   write(iout,'(/,A,I0)') error_warn//' invalid scan_itype=',scan_itype
   stop
  end select
 end do ! for i
