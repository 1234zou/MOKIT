...
program main
 ...
 if(i == 3) then
  ...
  if(idx1<1 .or. idx2<1 .or. idx1>idx2) then
   write(iout,'(A)') 'ERROR in subroutine fch_mo_copy: invalid indices idx1 and/or idx2.'
...
subroutine fch_mo_copy(fchname1, fchname2, ab, idx1, idx2)
 ...
 if(nbf/=nbf2 .or. nif/=nif2) then
  write(iout,'(A)') 'ERROR in subroutine fch_mo_copy: nbf and/or nif not equal in two files.'
  write(iout,'(A)') 'File1: '//TRIM(fchname1)//', File2: '//TRIM(fchname2)
  stop
 end if
