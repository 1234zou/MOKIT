...
subroutine gvb_sort_pairs(datname, nbf, nif, nocc, nopen, npair)
 ...
 if(buf(2:5) == '$SCF') then
  write(fid,'(1X,2(A,F16.12))') '$SCF    CICOEF(  1)=', pair_coeff(1,1), ',', pair_coeff(2,1)
 else
  write(fid,'(3X,2(A,F16.12))') 'CICOEF(  1)=', pair_coeff(1,1), ',', pair_coeff(2,1)
 end if
 do i = 2, npair, 1
  write(fid,'(3X,A,I3,2(A,F16.12))') 'CICOEF(',2*i-1,')=',pair_coeff(1,i),',',pair_coeff(2,i)
 end do
 write(fid,'(1X,A)') '$END'
 ! print done
