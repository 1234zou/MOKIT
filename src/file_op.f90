...
! simplify a Dalton .mol file generated by fch2dal, assuming the same basis set
! is used for all atoms of an elements
subroutine simplify_dalton_mol(molname)
 implicit none
 integer :: i, j, k, m, natom, natmtyp, fid, fid1, RENAME
 integer, allocatable :: ielem(:), nline(:), ntimes(:)
...
!f2py intent(in) :: molname
 ...
 i = LEN_TRIM(molname)
 if(molname(i-3:i) /= '.mol') then
  write(6,'(/,A)') 'ERROR in subroutine simplify_dalton_mol: the suffix of your&
                   & input filename'
  write(6,'(A)') 'must be .mol'
  stop
 end if
 new_mol = molname(1:i-4)//'.t'
 ...
 if(i /= 0) then
  write(6,'(/,A)') "ERROR in subroutine simplify_dalton_mol: string 'AtomTypes=&
                   &' not found"
 ...
 read(buf(11:),*) natom
 if(natom == 1) then ! no need to simplify
  close(fid)
  return
 end if

 allocate(ielem(natom), nline(natom), ntimes(natom), elem(natom), coor(3,natom))
 nline = 0; ntimes = 0; elem = ' '; coor = 0d0; eof = .false.
 read(fid,'(A)') buf

 do i = 1, natom, 1
  k = INDEX(buf,'.')
  read(buf(8:k-1),*) ielem(i)
 ...
 natmtyp = 1
 do i = 2, natom, 1
  if(ANY(ielem(1:i-1) == ielem(i))) cycle
  natmtyp = natmtyp + 1
 end do ! for i
 ...
 do i = 1, natom, 1
  ...
  ntimes(i) = COUNT(ielem == ielem(i))
 end do ! for i

 write(6,'(A,I0)') 'natmtyp=', natmtyp
 write(6,'(A)') 'ntimes='
 write(6,'(10I3)') ntimes
 rewind(fid)
 ...
 k = INDEX(buf, ' ')
 write(fid1,'(A,I0,A)') buf(1:10),natmtyp,TRIM(buf(k:))
 ...
 do i = 1, natom, 1
  if(ntimes(i) > 0) then
   ...
   k = INDEX(buf, ' Basis')
   m = LEN_TRIM(buf)
   write(fid1,'(A,I0,A)') buf(1:j+5), ntimes(i), buf(k:m)
   write(fid1,'(A2,3(1X,F18.8))') elem(i), coor(:,i)
   do j = i+1, natom, 1
    if(ielem(j) == ielem(i)) then
 ...
 deallocate(ielem, nline, ntimes, elem, coor)
 close(fid, status='delete')
 write(fid1,'(A)',advance='no')
 close(fid1)
 i = RENAME(TRIM(new_mol), TRIM(molname))
end subroutine simplify_dalton_mol
