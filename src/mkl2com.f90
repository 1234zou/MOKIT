! written by jxzou at 20230727: a wrapper of mkl2fch and fch2com
...
program mkl2com
...
#endif
 if(i /= 0) then
  write(6,'(/,A)') 'ERROR in program mkl2com: failed to call utility mkl2fch.'
  write(6,'(A)') 'mkl2com is a wrapper of mkl2fch and fch2com, so mkl2fch is re&
                 &quired to be'
 ...
 i = index(mklname, '.mkl', back=.true.)
 a_file = mklname(1:i-1)//'.a'
 b_file = mklname(1:i-1)//'.b'
 call lower(a_file)
 call lower(b_file)
