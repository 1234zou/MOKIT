...
! Generate NOs using AO overlap and AO density matrix in .json file. The "MOs"
! section will be updated by NO coefficients, and "Occupancy" will be updated
! by NO occupation numbers.
subroutine gen_no_using_orca_json(jsonfile, dm_type)
 implicit none
 integer :: nbf, nif
 integer, intent(in) :: dm_type
!f2py intent(in) :: dm_type
 real(kind=8), allocatable :: noon(:), ao_ovlp(:,:), dm(:,:), no_coeff(:,:)
 character(len=10) :: key
 character(len=240), intent(in) :: jsonfile
!f2py intent(in) :: jsonfile

 call read_nbf_and_nif_from_orca_json(jsonfile, nbf, nif)
 allocate(ao_ovlp(nbf,nbf))
 call read_square_array_from_orca_json(jsonfile, 'S-Matrix', nbf, ao_ovlp)
 allocate(dm(nbf,nbf))
 ...
 case(6) ! unrelaxed (total) density generated by AUTOCI module
  key = 'autocipur'
 case default
  write(6,'(/,A,I0)') 'ERROR in subroutine gen_no_using_orca_json: dm_type = ', &
                       dm_type
  write(6,'(A)') 'Only dm_type=1~6 are allowed in this subroutine.'
  stop
 end select

 call read_square_array_from_orca_json(jsonfile, TRIM(key), nbf, dm)
 allocate(noon(nif), no_coeff(nbf,nif))
 call gen_no_from_dm_and_ao_ovlp(nbf, nif, dm, ao_ovlp, noon, no_coeff)
 deallocate(ao_ovlp, dm)
 call write_mo_and_on_into_json(jsonfile, nbf, nif, no_coeff, noon)
 deallocate(noon, no_coeff)
end subroutine gen_no_using_orca_json

! Fake an ORCA .mdci.nat file from a .gbw file. Note: ORCA .densities and
! .densitiesinfo files are needed for this subroutine.
subroutine fake_mdci_nat_using_gbw(gbwname, dm_type)
 use util_wrapper, only: gbw2json, json2gbw
 implicit none
 integer :: i
 integer, intent(in) :: dm_type
!f2py intent(in) :: dm_type
 character(len=240) :: jsonfile, mdci_nat
 character(len=240), intent(in) :: gbwname
!f2py intent(in) :: gbwname

 call find_specified_suffix(gbwname, '.gbw', i)
 jsonfile = gbwname(1:i-1)//'.json'
 mdci_nat = gbwname(1:i-1)//'.mdci.nat'

 call gbw2json(gbwname, .true., dm_type)
 call gen_no_using_orca_json(jsonfile, dm_type)
 call json2gbw(jsonfile, mdci_nat)
 call delete_file(TRIM(jsonfile))
end subroutine fake_mdci_nat_using_gbw
