...
subroutine bas_gms2py(inpname, cart)
 ...
 logical :: alive, bohrs, ecp, uhf, lin_dep
 ...
 ! find the number of MOs (nif) and basis functions (nbf)
 rewind(inpid)
 do while(.true.)
  read(inpid,'(A)',iostat=i) buffer
  if(i /= 0) exit
  call upper(buffer)
  j = index(buffer,'$GUESS'); k = index(buffer,'NORB=')
  if(j/=0 .and. k/=0) then
   read(buffer(k+5:),*) nif
   exit
  end if
 ...
 if(i /= 0) then
  write(iout,'(A)') 'ERROR in subroutine bas_gms2py: No $GUESS section found&
 ...
 ! the Title Card line and the Point Group line
 read(inpid,'(A)') buffer
 i = index(buffer,'nbf=')
 if(i == 0) then
  write(iout,'(A)') 'ERROR in subroutine bas_gms2py: no nbf found in file '//TRIM(inpname)
  stop
 end if
 read(buffer(i+4:),*) nbf
 if(nbf > nif) then
  ...
  write(iout,'(A)') 'ERROR in ERROR in subroutine bas_gms2py: nbf<nif. This is impossible.'
  close(inpid)
  stop
 end if
 read(inpid,'(A)') buffer
