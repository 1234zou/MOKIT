...
def get_bfirst_from_mol(mol):
    '''
    Get the integer array bfirst from the mol/cell object. It contains the first
    indices for basis set of each atom.
    '''
    bfirst = np.ones(mol.natm+1, dtype=np.int32)
...
def mo_fch2py(fchname):
    ...
    '''
    from mokit.lib.fch2py import fch2py_cghf
...
def loc_driver(mol, mo, method='pm', lmo_ini=None, ao_ovlp=None, ao_dip=None,
               dis_tol=17.0, conv_tol=1e-5):
    '''
    TODO: can lmo_ini and be optional?
    '''
...
def loc(fchname, idx, method='pm', alpha=True, center_xyz=None, ions_centers=False,
    ...
    lmo = loc_driver(mol, mo[:,idx], method=method, ao_dip=ao_dip, dis_tol=dis_tol,
                     conv_tol=conv_tol)
...
def pbc_loc(molden, box, method='berry', wannier_xyz=None, ions_centers=False,
    ...
    bfirst = get_bfirst_from_mol(cell)
...
def get_dipole(fchname, itype=1):
    ...
    from mokit.lib.rwgeom import get_nuc_dipole
...
def nio(n_fch, n_1_fch):
    ...
    from mokit.lib.rwwfn import read_dm_from_fch, gen_no_from_dm_and_ao_ovlp, \
        check_uhf_in_fch, write_dm_into_fch
